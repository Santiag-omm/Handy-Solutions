...
class ServicioSeeder extends Seeder
{
    public function run(): void
    {
        ...
        if (Servicio::count() > 0) {
            $this->command->info('La tabla de servicios ya contiene datos. Omitiendo seeder.');
            return;
        }
        ...
        foreach ($servicios as $s) {
            Servicio::create(array_merge($s, [
                'slug' => Str::slug($s['nombre']),
                'activo' => true
            ]));
        }

        $this->command->info('Se han creado ' . count($servicios) . ' servicios iniciales.');
    }
}
